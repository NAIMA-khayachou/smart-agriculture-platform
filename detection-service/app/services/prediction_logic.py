import torch
from app.services.resnet_service import model, device
...
CLASSES = [
    ...
    'Chili__healthy', 'Chili__leaf curl', 'Chili__leaf spot',
    'Chili__whitefly', 'Chili__yellowish',
    'Coffee__cercospora_leaf_spot', 'Coffee__healthy',
    'Coffee__red_spider_mite', 'Coffee__rust',
    'Corn__common_rust', 'Corn__gray_leaf_spot',
    'Corn__healthy', 'Corn__northern_leaf_blight',
    'Cucumber__diseased', 'Cucumber__healthy',
    'Gauva__diseased', 'Gauva__healthy',
    'Grape__black_measles', 'Grape__black_rot',
    ...
    'Wheat__brown_rust', 'Wheat__healthy',
    'Wheat__septoria', 'Wheat__yellow_rust'
]

def format_result(class_name: str, confidence: float) -> dict:
    ...
    return {
        'prediction' : f'{plant} — {"Saine" if is_healthy else "Malade"}',
        'detail'     : f'La plante {plant} est en bonne santé' if is_healthy
                       else f'Maladie détectée : {condition}',
...
def predict(file_bytes: bytes) -> dict:
    tensor = process_image(file_bytes).to(device)
    ...
    return format_result(class_name, confidence)